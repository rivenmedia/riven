...
def _store_item(item: MediaItem):
    if isinstance(item, (Movie, Show, Season, Episode)) and item._id is not None:
        with db.Session() as session:
            session.merge(item)
            session.commit()
    else: _check_for_and_run_insertion_required(item)

def _get_item_from_db(item: MediaItem):
...
def _check_for_and_run_insertion_required(item: MediaItem) -> None:
    ...
    if input_item is not None:
        with db.Session() as session:
            if isinstance(input_item, (Movie, Show, Season, Episode)):
                item = input_item
                if not _check_for_and_run_insertion_required(item): 
                    item = _get_item_from_db(item)
