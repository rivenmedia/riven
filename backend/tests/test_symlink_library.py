...
class MockSettings:
    def __init__(self, library_path):
        # library_path and rclone_path must be of type(Path)
...
def test_invalid_library_structure(fs):
    ...
    # Then: It should fail initialization
    assert not library.initialized, "Library should fail initialization with incorrect structure."


def test_movie_detection(symlink_library, fs):
    # Given: A library with movies
    ...
    # Then: It should detect the movies
    assert len(movies) == 1, "Should detect two movies."
...
def test_show_detection(symlink_library, fs):
    # Given: A library with a show
    shows_path = "/fake/library/shows"
    fs.create_dir(f"{shows_path}/Vikings (2013) tt2306299")
    ...
    # Then: It should detect the show
    assert len(shows) == 1, "Should detect one show."
    assert isinstance(shows[0], Show), "Detected object should be of type Show."
...
def test_season_detection(symlink_library, fs):
    # Given: A library with a show's season
    shows_path = "/fake/library/shows"
    fs.create_dir(f"{shows_path}/Vikings (2013) tt2306299")
    fs.create_dir(f"{shows_path}/Vikings (2013) tt2306299/Season 01")
    fs.create_file(f"{shows_path}/Vikings (2013) tt2306299/Season 01/Vikings (2013) - s01e01 - Rites of Passage.mkv")
    ...
    # Then: It should detect the season
...
def test_episode_detection(symlink_library, fs):
    # Given: A library with a show's episode
    shows_path = "/fake/library/shows"
    fs.create_dir(f"{shows_path}/Vikings (2013) tt2306299")
    fs.create_dir(f"{shows_path}/Vikings (2013) tt2306299/Season 01")
    fs.create_file(f"{shows_path}/Vikings (2013) tt2306299/Season 01/Vikings (2013) - s01e01 - Rites of Passage.mkv")
    ...
    # Then: It should detect the episode
...
def test_media_item_creation(symlink_library, fs):
    # Given: A library with a valid movie entry
    fs.create_file("/fake/library/movies/Top Gun (1986) {imdb-tt0092099}/Top Gun (1986) {imdb-tt0092099}.mkv")

    # When: The library is scanned
    items = list(symlink_library.run())

    # Then: It should create a media item
