...
class EventManager:
    ...
    def __init__(self):
        ...
        self._paused_items: set[str] = set()  # Store paused item IDs
    ...
    def pause_queue(self, item_id: str = None):
        """
        Pause processing of events.
        
        Args:
            item_id (str, optional): If provided, only pause this specific item.
                                   If None, pause all queue processing.
        """
        with self.mutex:
            if item_id:
                self._paused_items.add(item_id)
                logger.info(f"Paused processing for item {item_id}")
            else:
                self._globally_paused = True
                logger.info("Queue processing globally paused")

    def resume_queue(self, item_id: str = None):
        """
        Resume processing of events.
        
        Args:
            item_id (str, optional): If provided, only resume this specific item.
                                   If None, resume all queue processing.
        """
        with self.mutex:
            if item_id:
                self._paused_items.discard(item_id)
                logger.info(f"Resumed processing for item {item_id}")
            else:
                self._globally_paused = False
                self._paused_items.clear()  # Clear all individual pauses when globally resuming
                logger.info("Queue processing globally resumed")

    def is_paused(self, item_id: str = None) -> bool:
        """
        Check if queue processing is paused.

        Args:
            item_id (str, optional): If provided, check if this specific item is paused.
                                   If None, check if queue is globally paused.

        Returns:
            bool: True if queue/item is paused, False otherwise.
        """
    ...
    def get_paused_items(self) -> list[str]:
        """
        Get list of paused item IDs.

        Returns:
            list[str]: List of paused item IDs.
        """
        return list(self._paused_items)
    ...
    def next(self):
        ...
        Raises:
            Empty: If the queue is empty or processing is paused.
        ...
        while True:
            ...
            if self._queued_events:
                with self.mutex:
                    ...
                    next_event = self._queued_events[0]
                    # Skip paused items but keep them in queue
                    if next_event.item_id in self._paused_items:
                        # If the first item is paused, check if there are any non-paused items
                        for event in self._queued_events:
                            if event.item_id not in self._paused_items:
                                if datetime.now() >= event.run_at:
                                    self._queued_events.remove(event)
                                    return event
                        raise Empty
                    if datetime.now() >= next_event.run_at:
