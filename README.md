...
- `episodes` - List of episode numbers (for multi-episode files)
...
- `is_remux`, `is_proper`, `is_repack`, `is_extended`, `is_directors_cut` - Boolean versions (True/False)
...
For files containing multiple episodes, use `{episodes[0]}` and `{episodes[-1]}`:

```json
{
  "episode_file_template": "s{season:02d}e{episodes[0]:02d}-e{episodes[-1]:02d}"
}
```

Results in: `s01e01-e02.mkv` for a file containing episodes 1 and 2.
