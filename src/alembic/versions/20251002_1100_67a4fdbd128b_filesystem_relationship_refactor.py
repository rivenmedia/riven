...
def upgrade() -> None:
    ...
    if "media_item_id" not in columns:
        op.add_column(
            "FilesystemEntry", sa.Column("media_item_id", sa.Integer(), nullable=True)
        )
    ...
    if "filesystem_entry_id" in mediaitem_columns:
        ...
        if "fk_mediaitem_filesystem_entry_id" in constraints:
            op.drop_constraint(
                "fk_mediaitem_filesystem_entry_id", "MediaItem", type_="foreignkey"
            )
    ...
    if "ix_filesystementry_media_item_id" not in indexes:
        op.create_index(
            "ix_filesystementry_media_item_id", "FilesystemEntry", ["media_item_id"]
        )
...
def downgrade() -> None:
    ...
    if "original_folder" not in fs_columns:
        op.add_column(
            "FilesystemEntry", sa.Column("original_folder", sa.String(), nullable=True)
        )
    ...
    if "filesystem_entry_id" not in mediaitem_columns:
        op.add_column(
            "MediaItem", sa.Column("filesystem_entry_id", sa.Integer(), nullable=True)
        )
    ...
    if "fk_filesystementry_media_item_id" in fs_constraints:
        op.drop_constraint(
            "fk_filesystementry_media_item_id", "FilesystemEntry", type_="foreignkey"
        )
