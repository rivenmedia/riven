...
def upgrade() -> None:
    ...
    if "FilesystemEntry" not in existing_tables:
        op.create_table(
            ...
            sa.CheckConstraint("file_size >= 0", name="ck_filesystem_entry_file_size_nonneg"),
    ...
    if "FilesystemEntry" in existing_tables:
        existing_ucs = {uc.get("name") for uc in inspector.get_unique_constraints("FilesystemEntry")}
        existing_ixs = {ix.get("name") for ix in inspector.get_indexes("FilesystemEntry")}
    ...
    if (
        "fk_mediaitem_filesystem_entry_id" not in mediaitem_fks and 
        "filesystem_entry_id" in {col['name'] for col in inspector.get_columns('MediaItem')}
    ):
