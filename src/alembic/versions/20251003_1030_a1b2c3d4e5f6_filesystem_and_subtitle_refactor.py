...
def upgrade() -> None:
    ...
    if 'Subtitle' in existing_tables:
        ...
        # Drop constraints
        subtitle_constraints = [c['name'] for c in inspector.get_unique_constraints('Subtitle')]
    ...
    existing_constraints = {c['name'] for c in inspector.get_unique_constraints('FilesystemEntry')}
    ...
    if 'ix_filesystem_entry_entry_type' not in existing_indexes:
        try:
            op.create_index('ix_filesystem_entry_entry_type', 'FilesystemEntry', ['entry_type'])
        except Exception:
            pass

    if 'ix_filesystem_entry_media_item_id' not in existing_indexes:
        try:
            op.create_index('ix_filesystem_entry_media_item_id', 'FilesystemEntry', ['media_item_id'])
        except Exception:
            pass
    ...
    if 'SubtitleEntry' in existing_tables:
        existing_fks = {fk['name'] for fk in inspector.get_foreign_keys('SubtitleEntry')}
        existing_indexes = {idx['name'] for idx in inspector.get_indexes('SubtitleEntry')}
        ...
        if 'ix_subtitle_entry_language' not in existing_indexes:
            try:
                op.create_index('ix_subtitle_entry_language', 'SubtitleEntry', ['language'])
            except Exception:
                pass

        if 'ix_subtitle_entry_file_hash' not in existing_indexes:
            try:
                op.create_index('ix_subtitle_entry_file_hash', 'SubtitleEntry', ['file_hash'])
            except Exception:
                pass

        if 'ix_subtitle_entry_opensubtitles_id' not in existing_indexes:
            try:
                op.create_index('ix_subtitle_entry_opensubtitles_id', 'SubtitleEntry', ['opensubtitles_id'])
            except Exception:
                pass
    ...
    if 'FilesystemEntry_old' in existing_tables:
        # Drop indexes
        old_indexes = [idx['name'] for idx in inspector.get_indexes('FilesystemEntry_old')]
        ...
        # Drop constraints
        old_constraints = [c['name'] for c in inspector.get_unique_constraints('FilesystemEntry_old')]
        for constraint_name in old_constraints:
            try:
                op.drop_constraint(constraint_name, 'FilesystemEntry_old', type_='unique')
            except Exception:
                pass

        # Drop foreign keys
        old_fks = [fk['name'] for fk in inspector.get_foreign_keys('FilesystemEntry_old')]
