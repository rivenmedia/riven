...
class AuthStartPost200Response(BaseModel):
    ...
    token: Optional[StrictStr] = Field(
        default=None, description="Authentication token to use in the `poll` endpoint"
    )
    confirm_url: Optional[StrictStr] = Field(
        default=None,
        description="URL where the user can confirm the authentication request",
    )
    __properties: ClassVar[List[str]] = ["token", "confirm_url"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate(
            {"token": obj.get("token"), "confirm_url": obj.get("confirm_url")}
        )
        return _obj
