...
class AuthStartPostRequest(BaseModel):
    ...
    email_confirmation: Optional[StrictBool] = Field(
        default=True, description="Whether to email the user a confirmation link"
    )
        ...
        _obj = cls.model_validate(
            {
                "email": obj.get("email"),
                "email_confirmation": (
                    obj.get("email_confirmation")
                    if obj.get("email_confirmation") is not None
                    else True
                ),
            }
        )
