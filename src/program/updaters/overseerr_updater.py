...
class OverseerrUpdater:
    ...
    def run(self, item: MediaItem):
        ...
        if parent.last_overseerr_status is not None and parent.last_overseerr_status != parent.overseerr_status.name:
            logger.debug(f"PARENT: {parent.log_string}: {parent.last_overseerr_status} -> {parent.overseerr_status.name}")
            if parent.overseerr_status == OverseerrStatus.Available:
                ...
                if Overseerr.mark_partially_available(item.overseerr_id):
                    parent.last_overseerr_status = OverseerrStatus.PartiallyAvailable.name
                else:
                    self.pending.add(parent)
        ...
        if item == parent:
            ...
            if isinstance(item, Season):
                for ep in item.episodes:
                    if ep.last_overseerr_status is not None and ep.last_overseerr_status != ep.overseerr_status.name:
                        logger.debug(f"{ep.log_string}: {ep.last_overseerr_status} -> {ep.overseerr_status.name}")
                        ep.last_overseerr_status = ep.overseerr_status.name
            # don't interfere with rest of the process
            logger.debug(f"ITEM: {item.log_string}: {item.last_overseerr_status} -> {item.overseerr_status.name}")
