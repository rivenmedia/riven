...
class Symlinker:
    ...
    def _calculate_next_attempt(self, item: Union[Movie, Show, Season, Episode]) -> datetime:
        if item.symlinked_times < 5:
            next_attempt_delay = timedelta(seconds=5 * (2 ** item.symlinked_times))
        elif item.symlinked_times == 5:
            next_attempt_delay = timedelta(minutes=5)
        elif item.symlinked_times == 6:
            next_attempt_delay = timedelta(minutes=10)
        else:
            next_attempt_delay = timedelta(minutes=20)
