...
class MdblistRequestHandler(BaseRequestHandler):
    ...
    def execute(self, method: HttpMethod, endpoint: str, ignore_base_url: bool = False, **kwargs) -> ResponseObject:
        return super()._request(method, endpoint, ignore_base_url=ignore_base_url, **kwargs)
...
class MdblistAPI:
    ...
    def __init__(self, api_key: str):
        ...
        self.request_handler = MdblistRequestHandler(session, base_url=self.BASE_URL)

    def validate(self):
        return self.request_handler.execute(HttpMethod.GET, f"api/user?apikey={self.api_key}")

    def my_limits(self):
        """Wrapper for mdblist api method 'My limits'"""
        response = self.request_handler.execute(HttpMethod.GET,f"api/user?apikey={self.api_key}")
        return response.data

    def list_items_by_id(self, list_id: int):
        """Wrapper for mdblist api method 'List items'"""
        response = self.request_handler.execute(HttpMethod.GET,f"api/lists/{str(list_id)}/items?apikey={self.api_key}"
                       )
        return response.data
    ...
    def list_items_by_url(self, url: str):
        ...
        response = self.request_handler.execute(HttpMethod.GET, url, ignore_base_url=True, params={"apikey": self.api_key})
        return response.data